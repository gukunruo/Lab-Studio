...
import { buildAnthropicWebSearchTools, type AnthropicSearchConfig } from './web-search'
...
function imageEnrichHeaders(config: AnthropicSearchConfig): Headers {
  return new Headers({
...
export async function enrichImagePrompt(
  ...
  try {
    const request = buildImageEnrichmentRequest(prompt, config, maxSearchUses)
