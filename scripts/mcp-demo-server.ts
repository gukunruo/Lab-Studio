...
// 一个 McpServer 只能连接到一个 transport（SDK 报 "Already connected to a transport"），
// 所以每个会话新建一个 server + transport，会话关闭时随 map 一起回收。工具注册走工厂函数。
...
// 有状态会话：按 mcp-session-id 复用同一个 transport + server，关闭时清理。
// stateless 模式不允许复用同一个 transport 处理多次请求，因此显式维护会话表。
...
async function handleMcp(req: Request): Promise<Response> {
  ...
  created = new WebStandardStreamableHTTPServerTransport({
    ...
    onsessioninitialized: (id) => { sessions.set(id, { transport: created, server }) },
    onsessionclosed: (id) => { sessions.delete(id); void server.close().catch(() => {}) },
  })
  await server.connect(created)
  return created.handleRequest(req)
}
