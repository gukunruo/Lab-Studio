...
const exp = computed(() => worlds.find((e) => e.slug === props.slug))
...
watch(
  () => props.slug,
  (slug) => {
    const found = worlds.find((e) => e.slug === slug)
...
<template>
  <div v-if="exp" ref="expRoot" class="wv" :class="{ 'wv--fullscreen': isFullscreen }">
    <div v-if="isFullscreen" class="wv__hover-zone" />
    <div class="wv__bar">
      <RouterLink to="/3d" class="wv__back">{{ i18n.t('world.back') }}</RouterLink>
      <div class="wv__meta">
        <h1 class="wv__title">{{ i18n.tl(exp.title) }}</h1>
        <span class="wv__slug">{{ exp.slug }}</span>
      </div>
      <div class="wv__actions">
        <button
          v-if="exp.doc"
          class="wv__doc-btn"
          @click="showDoc = true"
...
.wv--fullscreen:has(.wv__hover-zone:hover) .wv__bar,
.wv--fullscreen .wv__bar:hover {
  opacity: 1;
  pointer-events: auto;
}

.wv--fullscreen .wv__hover-zone {
  display: block;
  position: absolute;
  top: 0;
  left: 0;
...
.wv--fullscreen .wv__fs-exit:hover {
  background: rgba(0, 0, 0, 0.6);
  color: #fff;
}

.wv--fullscreen .wv__stage {
  flex: 1;
  min-height: 0;
  border: none;
  border-radius: 0;
