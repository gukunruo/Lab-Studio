...
export interface WorldMeta extends Required<Omit<WorldMetaInput, 'icon'>> {
  slug: string
  icon?: string
...
const metaModules = import.meta.glob<WorldMetaInput>('./*/meta.ts', {
  eager: true,
  import: 'default',
})
...
export const worlds: WorldMeta[] = Object.entries(metaModules)
  .map(([path, meta]): WorldMeta | null => {
  ...
  .filter((e): e is WorldMeta => e !== null)
