...
![MCP Architecture](/learn-claude-code/assets/s19-mcp-architecture.svg)
...
| assemble\_tool\_pool | 把内置工具和 MCP 工具组装成一个工具池 |
...
s10-s18 的 agent\_loop 用 prompt cache 避免重复序列化。s19 去掉了缓存：
...
教学版中，`connect_mcp` 是 Lead 工具，`assemble_tool_pool` 也只服务于 Lead 的 agent\_loop。Teammate 仍使用固定的 8 个子集工具（bash、read\_file、write\_file、send\_message、submit\_plan、list\_tasks、claim\_task、complete\_task）。
...
| 工具池 | 固定 BUILTIN\_TOOLS | assemble\_tool\_pool 动态组装 mcp\_\_ 前缀工具 |
| 名称安全 | 无 | normalize\_mcp\_name 规范化 |
...
| Lead 工具 | 17 (s18) | 18 (+connect\_mcp) |
...
- 过期前 5 分钟自动刷新
- 支持跨应用访问（XAA）：浏览器获取 id\_token → RFC 8693 + RFC 7523 交换 → 无需反复弹浏览器
