...
![Background Tasks Overview](/learn-claude-code/assets/s13-background-tasks-overview.svg)
...
慢操作先回一个带 `bg_id` 的占位 tool\_result，LLM 知道这个命令还在跑，可以先做别的事。后台完成后，通知作为独立 text block 和当前轮的 tool\_result 一起组成 user 消息。
...
| 通知格式 | — | `<task_notification>`（不复用 tool\_use\_id） |
