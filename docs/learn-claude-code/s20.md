...
![System Architecture](/learn-claude-code/assets/s20-system-architecture.svg)
...
循环本身仍然是同一个结构：调用模型，检查响应里是否出现 `tool_use` block，执行工具，把结果追加回 `messages`。CC 源码里也不直接信任 `stop_reason == "tool_use"`，而是以实际出现的 tool\_use block 作为是否继续工具轮的信号。变化的是循环周围的 harness 变完整了。
...
| LLM 前 | compaction pipeline | 先压大输出，再裁历史，再压旧 tool\_result，必要时摘要 |
...
| 返回循环 | tool\_result | 每个 `tool_use` 对应一个 `tool_result`，再回到下一轮 |
| 本轮没有 tool\_use / 停止时 | `Stop` hooks | 统计、清理、审计 |
...
| error recovery | 简化 try/except | retry / max\_tokens / prompt too long |
