...
![Agent Loop](/learn-claude-code/assets/s01-agent-loop.svg)
...
深入 CC 源码
> 以下内容基于 CC 源码 `src/query.ts`（1729 行）的核查。核心差异就两个：CC 不看 `stop_reason` 字段而是检查内容里有没有 tool\_use 块（因为流式响应中 stop\_reason 不可靠）；CC 有更多的退出路径和恢复策略做生产级保护。
