...
![Task System Overview](/learn-claude-code/assets/s12-task-system-overview.svg)

教学代码保留基础 agent loop，为聚焦任务系统省略了 S11 的完整错误恢复（RecoveryState、退避、升级、reactive compact、fallback model）。新增 5 个任务工具 + `.tasks/` 目录持久化 + `blockedBy` 依赖检查。任务系统与错误恢复是独立层：CC 源码中 `utils/tasks.ts` 只管 CRUD，`query.ts` 的 with\_retry/RecoveryState 管错误恢复，互不耦合。
...
| 状态 | pending / in\_progress / completed | pending / in\_progress / completed |
...
![Task DAG](/learn-claude-code/assets/s12-task-dag.svg)
...
- **complete\_task**: `in_progress` → `completed`。把任务标记为完成，并解锁下游。
...
| 工具 | bash, read\_file, write\_file (3) | + create\_task, list\_tasks, get\_task, claim\_task, complete\_task (8) |
| 生命周期 | — | pending → in\_progress → completed（无 release 回退） |
...
注意：教学版把 claim 和开始工作合成一步（claim = set owner + in\_progress）；真实 CC 的 `claimTask` 主要解决 owner 竞争，只设 owner 不改 status，状态更新由 `TaskUpdate` 完成。
