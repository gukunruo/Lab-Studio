...
应用注册表中的 `entry` 决定卡片点击后的入口：

- `frame`：进入通用实验外壳，适合 `hello-lab` 或仍需要介绍、说明和实验上下文的项目。
- `direct`：跳过通用中间页，直接进入项目的主要使用界面。当前音乐播放器使用此模式，点击卡片后直接打开全屏播放器。

新项目默认使用 `frame`。只有项目已经具备完整、明确的主界面，并且中间页不会提供额外价值时，才设置为 `direct`。
